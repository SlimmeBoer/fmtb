...
export default function Admin_KLWData() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await axiosClient.get('/klwdump');
    ...
    return (
        <Box sx={{width: '100%', maxWidth: {sm: '100%', md: '1700px'}}}>
            ...
            {!loading && companies.length === 0 &&
                <Typography variant="body2">{t("klw_overview.no_companies_in_db")}</Typography>}
            {!loading && companies.length !== 0 &&
                <TableContainer>
                    <Table size="small">
                        <TableHead>
                        ...
                        <TableBody>
                            {companies.map((company) => (
                                <TableRow key={company.id}>
                                    <TableCell><IconButton onClick={() => handleDeleteCompanyClick(company.id)}
                                                           aria-label="delete">
                                        <DeleteIcon/>
