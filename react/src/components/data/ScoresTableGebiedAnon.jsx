...
export default function ScoresTableGebiedAnon(props) {
    ...
    const getScores = () => {
        if (props.area !== '') {
            setLoading(true);
            axiosClient.get(`/kpi/getareascores/${props.area}`)
                .then(({data}) => {
                    setLoading(false);
                    setScores(data);
                })
                                ...
                                return (
                                    <TableRow key={index} sx={{margin: 0}}>
                                        <TableCell component="th" scope="row">
                                            {index+1}. <Link href={props.link + s.company_id}>Bedrijf {s.company_id}</Link>
                                        </TableCell>
